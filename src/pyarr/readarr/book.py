...
class Book(CommonActions):
    ...
    def get(self, item_id: int | None = None) -> JsonArray | JsonObject:
        ...
        """
        return self._get("book", item_id=item_id)
    ...
    def add(
        ...
        response = self.handler.request("book", method="POST", json_data=book)
        if isinstance(response, dict):
            return response
        raise ValueError("Expected a dictionary response from the 'book' endpoint")

    def update(self, book: JsonObject, editions: JsonArray) -> JsonObject:
        ...
        response = self.handler.request("book", method="PUT", json_data=book)
        if isinstance(response, dict):
            return response
        raise ValueError("Expected a dictionary response from the 'book' endpoint")

    def monitor(self, book_ids: list[int], monitored: bool = True) -> JsonArray:
        ...
        response = self.handler.request("book/monitor", method="PUT", json_data=json_data)
    ...
    def delete(
        ...
        self.handler.request(f"book/{item_id}", method="DELETE", params=params)
    ...
    def lookup(self, term: str) -> JsonArray:
        ...
        """
        response = self.handler.request("book/lookup", params={"term": term})
        if isinstance(response, list):
            return response
