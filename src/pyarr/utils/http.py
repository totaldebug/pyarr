...
class RequestHandler:
    ...
    def __init__(
        ...
        self.session: requests.Session | None = requests.Session()  # Initialize the session here

        if api_ver is None:
            api_ver = self._get_api_version()

        self.url = self.url.joinpath("api").joinpath(api_ver)
    ...
    def request(
        ...
        Returns:
            Any: The response data as a dictionary, list, or requests.Response object.
        """
        if params:
        ...
        url = self.url.joinpath(endpoint)
