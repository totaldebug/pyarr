...
class Series(CommonActions):
    ...
    def get(self, item_id: int | None = None, tvdb: bool = False, tmdb: bool = False) -> JsonArray | JsonObject:
        ...
        return self._get("series", item_id=item_id)
    ...
    def add(
        ...
        response = self.handler.request("series", method="POST", json_data=series)
    ...
    def update(self, data: JsonObject) -> JsonObject:
        ...
        """
        response = self.handler.request("series", method="PUT", json_data=data)
        if isinstance(response, dict):
            return response
        raise ValueError("Expected a dictionary response from the 'series' endpoint")
    ...
    def lookup(self, term: str | None = None, item_id: int | None = None) -> JsonArray:
        ...
        response = self.handler.request("series/lookup", params=params)
        if isinstance(response, list):
            return response
