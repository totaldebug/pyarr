...
class Movie(CommonActions):
    ...
    def get(self, item_id: int | None = None, tmdb_id: int | None = None) -> JsonArray | JsonObject:
        ...
        return self._get("movie", item_id=item_id, params=params)
    ...
    def add(
        ...
        response = self.handler.request("movie", method="POST", json_data=movie)
        if isinstance(response, dict):
            return response
        raise ValueError("Expected a dictionary response from the 'movie' endpoint")

    def update(self, data: JsonObject, move_files: bool | None = None) -> JsonObject:
        ...
        response = self.handler.request("movie", method="PUT", json_data=data, params=params)
        if isinstance(response, dict):
            return response
        raise ValueError("Expected a dictionary response from the 'movie' endpoint")
    ...
    def lookup(self, term: str) -> JsonArray:
        ...
        """
        response = self.handler.request("movie/lookup", params={"term": term})
