...
class TrackFile(CommonActions):
    ...
    def get(
        ...
        return self._get("trackfile", item_id=item_id, params=params)
    ...
    def update(self, data: JsonObject) -> JsonObject:
        ...
        """
        response = self.handler.request("trackfile", method="PUT", json_data=data)
    ...
    def delete(self, ids: int | list[int]) -> None:
        ...
        if isinstance(ids, list):
            ...
            self.handler.request("trackfile/bulk", method="DELETE", json_data=json_data)
        else:
            self._delete("trackfile", item_id=ids)
